        ...
        */
        ...
        foreach($results as $doc){
            if(in_array($doc->SKU, array_keys($artemp) )){
                ...
                $tdoc->unitId = $tdoc->unitId.'<br />'.$this->shortunit( array('unitId'=>$doc->unitId) ).'<i class="fa fa-remove-sign del_unit" id="'.$doc->unitId.'" ></i>';
                $artemp[$doc->SKU] = $tdoc;
            }else{
                $doc->quantity = 1;
                $doc->unitTotal = $doc->productDetail['priceRegular'];
                $doc->unitId = $this->shortunit( array('unitId'=>$doc->unitId) ).' <i class="fa fa-remove-sign del_unit" id="'.$doc->unitId.'" ></i>';
                $artemp[$doc->SKU] = $doc;
            }
        }
